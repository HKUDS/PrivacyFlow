...
(() => {
  ...
  const exact = new Map([
    ...
    ["一键接入 APG，并可完整恢复原配置", "Connect to APG in one step and restore the exact previous configuration"],
    ["将 Agent 接入 APG", "Connect agents to APG"],
    ["选择模型后，APG 会先验证 Agent 所需协议，再安全更新用户级配置。恢复时会完整还原接入前的文件。", "APG validates the protocol required by each agent before safely updating user-level configuration. Restore returns every file to its exact pre-connect state."],
    ...
    ["命令未出现在 APG 的 PATH 中", "The command is not on APG's PATH"],
    ["快速接入", "Quick connect"],
    ...
    ["已接入 APG", "connected to APG"],
    ["已恢复原配置", "restored its previous configuration"],
    ["确认迁移现有配置", "Confirm existing configuration migration"],
    ["已有配置", "Existing configuration"],
    ["APG 检测到这些文件中已有相关配置。确认后才会替换它们。", "APG found related configuration in these files. They will be replaced only after you confirm."],
    ["APG 会先保存接入前的完整文件和权限，接入后仍可使用“恢复原配置”精确还原。取消不会修改任何文件。", "APG first saves the complete pre-connect files and permissions. After connecting, Restore previous configuration can return them exactly. Cancel leaves every file unchanged."],
    ["确认迁移并接入", "Confirm migration and connect"],
    ["检测到已有 Agent 配置。确认迁移后，APG 会先保存完整快照再替换；取消则不改动。", "An existing Agent configuration was found. APG will save a complete snapshot before replacing it; cancel to leave it unchanged."],
    ...
    ["留空使用 APG 默认值", "Leave blank to use APG's default"],
