...
class MaterializationEngine:
    ...
    def materialize_placeholder(self, ph: ParsedPlaceholder, *, session_id: str, sink_type: str) -> MaterializationResult:
        if not self.signer.is_valid(ph):
            return MaterializationResult(False, None, self._code("PLACEHOLDER_INVALID_MAC"), False, "regenerate_context")
        ok, rec, code = self.store.validate_active(ph.handle_id, session_id, self.workspace_id)
        if not ok or rec is None:
            return MaterializationResult(False, None, self._normalize_code(code), False, "regenerate_context")
        ...
        if rec.value is None:
            return MaterializationResult(False, None, self._code("VALUE_UNAVAILABLE"), False, "ask_user_for_approval")
        if rec.kind == "path" and ph.suffix:
            joined = self.path_aliases.join_suffix(rec.value, ph.suffix)
            if joined is None:
                return MaterializationResult(False, None, self._code("PATH_TRAVERSAL_BLOCKED"), False, "regenerate_context")
