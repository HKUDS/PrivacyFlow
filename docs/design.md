...
Detector management is implemented as a small overlay rather than editing the primary YAML file. Preset selection, module enablement, and WebUI-created rules are persisted in a mode-`0600` JSON file beside the mapping database. The overlay is merged with the startup detector configuration and compiled into a replacement `DetectorManager`; new requests observe the replacement atomically while in-flight requests finish on their existing flow.
